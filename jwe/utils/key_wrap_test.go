...
func TestAesKeyWrap(t *testing.T) {
	...
	out0, _ := jweutils.KeyWrap(block0, cek0)
	out1, _ := jweutils.KeyWrap(block1, cek1)
	out2, _ := jweutils.KeyWrap(block2, cek2)

	require.Equal(t, expected0, out0)
	require.Equal(t, expected1, out1)
	require.Equal(t, expected2, out2)

	unwrap0, _ := jweutils.KeyUnwrap(block0, out0)
	unwrap1, _ := jweutils.KeyUnwrap(block1, out1)
	unwrap2, _ := jweutils.KeyUnwrap(block2, out2)
...
func TestAesKeyWrapInvalid(t *testing.T) {
	...
	_, err := jweutils.KeyUnwrap(block, input0)
	...
	_, err = jweutils.KeyUnwrap(block, input1)
	...
	_, err = jweutils.KeyWrap(block, input2)
